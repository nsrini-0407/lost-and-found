...
export default function AdminItemRow({ item }: AdminItemRowProps) {
  ...
  const handleReject = () => {
    const reason = prompt(
      'Optional: Enter rejection reason (sent to submitter)'
    );
  ...
  return (
    <tr className="hover:bg-slate-50 transition-colors">
      {/* Title — clicking opens the detail page */}
      <td className="px-4 py-3">
        <Link
          href={`/admin/items/${item.id}`}
          className="font-medium text-navy-800 leading-tight hover:text-amber
      ...
      <td className="px-4 py-3">
        <div className="text-slate-600 text-sm">{item.submitter_name}</div>
        ...
        >
          ...
          {status === 'approved' && (
            <button
              ...
              disabled={loading}
              className="px-2.5 py-1 bg-violet-50 text-violet-700 text-xs
                         font-semibold rounded-sm hover:bg-violet-100
